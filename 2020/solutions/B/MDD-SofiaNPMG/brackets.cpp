/// Скъпи читатели, тук ще прочетете
/// една много добре разяснена, твърде усложнена,
/// бавна и неработеща във всички случаи задача.
/// Приятно четене ;D
#include <iostream>
#include <stack>
#include <string>
...
int main(){
    ...
    for(int i=1; i<indexOfBracketInSymbol.size(); i++){ /// от i=1, за да прескочим -1
        ...
        if(importantSign[currentBracket] == 0 & importantSign[corespondingBracket[currentBracket]] == 0 ){ /// скобите не са нужни, след като нито общо са умножени по нещо, нито са разделени
            ...
            for(int j = i; indexOfBracketInSymbol[j]<corespondingBracket[currentBracket]; j++){
