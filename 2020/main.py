...
def compile_source(sourcePath, destPath):
    mainArgs = ['g++', '-O2', '-o', destPath, sourcePath]
    for std in STD:
        compileArgs = mainArgs + [('-std=' + std)]
        try:
...
def run_test(solution, test, task):
    ...
    args = ['isolate', '-i', 'input', '-o', 'output', '-r', '/dev/null',
            '-t', str(task['tl']), '-m',
...
def crawl_tasks(testGroups = 'ABCDE'):
    for group in os.listdir('problem-data'):
        Tasks[group] = {}
        if group not in testGroups:
            continue
        ...
        for task in list_sorted(os.path.join('problem-data', group)):
            ...
            for test in list_sorted(testPath):
                ...
                try:
                    ...
                    print('Warning {0} test not parsed for {1}-{2}'.format(
                        test, group, task)
                    )
                    continue
            ...
            Tasks[group][task]['tl'] = max(res['time'] * AUTHOR_TL_RATIO,
                    MIN_TL[group])
            if task in SUGGEST_TL:
                Tasks[group][task]['tl'] = max(Tasks[group][task]['tl'],
                        SUGGEST_TL[task]
                )
...
def test_solutions(testGroups = 'ABCDE'):
